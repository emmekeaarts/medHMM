...
dwell_distr <- dwell_start <- list(matrix(log(c(20, 1.5,
                                            10, 1.5,
                                            5, 1.5)), nrow = m, ncol = 2, byrow = TRUE))
...
medHMM_cont(s_data = sim_obs,
            ...
            show_progress = TRUE,
            mcmc = list(J = J, burn_in = B), return_path = TRUE, max_dwell = 100)
...
s_data = input_data_mhmm$observations
gen = list(m = m, n_dep = n_dep)
start_val = c(list(gamma2), list(emiss_distr_2nd_chain,emiss_distr2_2nd_chain), list(dwell_mu_var_2nd_chain))
emiss_hyp_prior = hyp_pr
dwell_hyp_prior = dwell_hyp_prior
...
medHMM_cont <- function(s_data, gen, xx = NULL, start_val, emiss_hyp_prior, dwell_hyp_prior,
    ...
    # logmu           <- matrix(log1p(PD[1, ((n_dep * m * 2 + m * m + 1)) :((n_dep * m * 2 + m * m + m))]), nrow = n_subj, ncol = m, byrow = TRUE)
...
gamma <- matrix(c(0.9, 0.1,
                  0.3, 0.7), nrow = m, ncol = m, byrow = TRUE)

emiss_distr <- list(matrix(c(10,2,
                             50,2), nrow = m, ncol = 2, byrow = TRUE),
                    matrix(c(-5,2,
                             -20,2), nrow = m, ncol = 2, byrow = TRUE))

dwell_distr <- dwell_start <- matrix(log(c(50,2,
                                           10,2)), nrow = m, ncol = 2, byrow = TRUE)
...
sim_data <- mHMMbayes::sim_mHMM(n_t = n_t, n = n, m = m, n_dep = n_dep, data_distr = 'continuous',
                                 gamma = gamma, emiss_distr = emiss_distr, var_gamma = .1, var_emiss = c(.5, 0.01), return_ind_par = TRUE)

# Specify hyper-prior for the continuous emission distribution
emiss_hyp_pr <- list(
    emiss_mu0 = list(matrix(c(10,50), nrow = 1),
                     matrix(c(-5, -20), nrow = 1)),
    emiss_K0  = list(1, 1),
    emiss_nu  = list(1, 1),
...
out <- medHMM_cont(s_data = sim_data$obs,
                   ...
                   dwell_hyp_prior = dwell_hyp_pr,
                   show_progress = TRUE,
                   mcmc = list(J = 500, burn_in = 250), return_path = TRUE, max_dwell = 120)
...
# Simulate data
set.seed(42)
sim_data_lnorm <- sim_data <- medHMM::sim_medHMM(n_t, n, data_distr = 'continuous', m, n_dep = n_dep,
                      dwell_distr = dwell_distr, dwell_type = 'lognormal',
                      start_state = NULL, q_emiss = NULL, gamma = gamma, emiss_distr = emiss_distr, xx_vec = NULL, beta = NULL,
                      var_gamma = 0.1, var_emiss = c(0.1,0.1), var_dwell = 0.01, return_ind_par = TRUE)
...
sim_data_lnorm$subject_dwell
sim_data_pois$subject_dwell
...
dwell_hyp_pr_plnorm <- list(
  ...
  # emiss_mu0 = rep(list(matrix(rep(1,m), nrow = 1)),n_dep),
  dwell_K0  = c(1),
  dwell_nu  = c(1),
  dwell_V   = rep(0.1, m)
)
...
library(MCMCpack)
# Run the model on the simulated data:
out_lognorm <- medHMM_cont(s_data = sim_data$obs,
                   gen = list(m = m, n_dep = n_dep),
                   start_val = c(list(gamma), emiss_distr, list(dwell_start)),
                   emiss_hyp_prior = emiss_hyp_pr,
                   dwell_hyp_prior = dwell_hyp_pr,
                   show_progress = TRUE,
                   mcmc = list(J = 500, burn_in = 350), return_path = TRUE, max_dwell = 200)


out_pois <- medHMM_cont_pois(s_data = sim_data$obs,
                   gen = list(m = m, n_dep = n_dep),
                   start_val = c(list(gamma), emiss_distr, list(matrix(exp(dwell_start[,1]), ncol = 1))),
                   emiss_hyp_prior = emiss_hyp_pr,
...
#
#
# gamma <- matrix(c(0, 0.6, 0.3, 0.1,
#                   0.4, 0, 0.3, 0.3,
#                   0.2, 0.3, 0, 0.5,
#                   0.3, 0.1, 0.6, 0), nrow = m, ncol = m, byrow = TRUE)
#
# emiss_distr <- list(matrix(c(10,2,
#                              50,2,
#                              2,2,
...
# # Specify hyper-prior for the continuous emission distribution
# emiss_hyp_pr <- list(
#     emiss_mu0 = list(matrix(c(10,50,2,-10), nrow = 1),
#                      matrix(c(-5, -20, 5, 20), nrow = 1)),
#     emiss_K0  = list(1, 1),
#     emiss_nu  = list(1, 1),
#     emiss_V   = list(rep(100, m), rep(100, m)),
#     emiss_a0  = list(rep(0.01, m), rep(0.01, m)),
...
apply(out$dwell_varmu_bar[301:500,], 2, mean)
...
#     mutate(subj = factor(subj)) %>%
#     gather(parameter, value, -iter, -subj) %>%
#     filter(str_detect(parameter, "dur_logmu")) %>%
#     ggplot(aes(iter, value, colour = subj)) +
#     geom_line() +
#     facet_wrap(parameter~.)
...
subj_data %>%
    mutate(subj = factor(subj)) %>%
    gather(parameter, value, -iter, -subj) %>%
    filter(str_detect(parameter, "dur_lambda")) %>%
    ggplot(aes(iter, value, colour = subj)) +
    geom_line() +
    facet_wrap(parameter~.)


apply(out_lognorm$dwell_mu_bar[301:500,],2,median)
apply(out_pois$dwell_mu_bar[301:500,],2,median)
apply(out_shiftpois$dwell_mu_bar[301:500,],2,median)
log(c(5,2,10))

out <- out_lognorm
subj_data_lognorm <- do.call(rbind, lapply(1:length(out$PD_subj), function(s) out$PD_subj[[s]] %>%
                                       as.data.frame() %>%
                                       mutate(subj = s,
                                              iter = row_number())))
dur_lognorm <- subj_data_lognorm %>%
    # mutate(subj = factor(subj)) %>%
    gather(parameter, value, -iter, -subj) %>%
    filter(iter > 300,
           str_detect(parameter, "dur_logmu")) %>%
    group_by(subj, parameter) %>%
    summarise(mu = exp(median(value))) %>%
    spread(parameter, mu) %>%
    ungroup() %>%
    as.matrix()
...
out <- out_pois
subj_data_pois<- do.call(rbind, lapply(1:length(out$PD_subj), function(s) out$PD_subj[[s]] %>%
                                               as.data.frame() %>%
                                               mutate(subj = s,
                                                      iter = row_number())))
dur_pois <- subj_data_pois %>%
    # mutate(subj = factor(subj)) %>%
    gather(parameter, value, -iter, -subj) %>%
    filter(iter > 300,
           str_detect(parameter, "dur_lambda")) %>%
    group_by(subj, parameter) %>%
...
true_dur <- cbind(1:30, do.call(rbind, lapply(sim_data$subject_dwell, function(s) s[,1])))


mean((as.numeric(dur_lognorm[,-1]) - as.vector(true_dur[,-1]))^2)
mean((as.numeric(dur_pois[,-1]) - as.vector(true_dur[,-1]))^2)


colMeans(((dur_lognorm[,-1]) - (true_dur[,-1])))
colMeans(((dur_pois[,-1]) - (true_dur[,-1])))




exp(apply(out$dwell_mu_bar[101:200,],2,median))

sim_data$states

data.frame("state" = rle(x = sim_data$states[,2])$values,
           "dwell" = rle(x = sim_data$states[,2])$lengths
           ) %>%
    group_by(state) %>%
    summarise("mean" = mean(dwell),
              "q50" = quantile(dwell, 0.5),
              "q80" = quantile(dwell, 0.8),
              "q90" = quantile(dwell, 0.9),
              "q95" = quantile(dwell, 0.95),
              "q99" = quantile(dwell, 0.99),
              "q100" = quantile(dwell, 1.0))

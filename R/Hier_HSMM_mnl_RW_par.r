...
Hier_HSMM_mnl_RW_par <- function(Data, Start_Val, Gamma_Sampler = NULL, Pr_Sampler = NULL, Gamma_hyp_Prior = NULL, Pr_hyp_Prior = NULL, D_hyp_Prior, Mcmc, Fast_vers = FALSE, Fast_vers_par = NULL){
...
# Run the MCMC algorithm
	...
	for (iter in 2 : J){	
		...
		for(mi in 1:n.mice){
			...
			Occupancy				<- FB[[mi]][[3]]
			N 						<- FB[[mi]][[1]]
			PD.mice[[mi]][iter, m*q_pr + m*m + m + 1] 	<- sum(N)		

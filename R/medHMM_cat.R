...
# logsigma					~ invGamma(alpha.simga20, beta.sigma20)
...
# Startvalues for logmu, logsigma (for first run FB), mu_d_bar, tau2_bar (to start gibbs sampler for duration distribution), gamma and pr (for first run FB), gamma and pr get their start values from the mle at the first run.
#     Note: now programmed to set startvalues logmu = startvalues mu_d_bar, and startvalues logsigma = startvalues tau2_bar.


medHMM_cat <- function(Data, Start_Val, Gamma_Sampler = NULL, Pr_Sampler = NULL, Gamma_hyp_Prior = NULL, Pr_hyp_Prior = NULL, D_hyp_Prior, Mcmc, Fast_vers = FALSE, Fast_vers_par = NULL){
  ...
  for (iter in 2 : J){
    ...
    for(mi in 1:n.mice){
      ...
      d 						<- get.d.lognorm(run.p = list(logmu = logmu[mi,], logsd = sqrt(logsigma2)), Mx = m.data[[mi]]$Mx, m = m)
      if (Fast_vers == FALSE){
        FB					<- FBalgC(y2 = m.data[[mi]]$y2, m = m, pr = pr[[mi]], Mx = m.data[[mi]]$Mx, Mx2 = m.data[[mi]]$Mx2, gamma = gamma[[mi]], d = d, S2 = m.data[[mi]]$switch2, S = m.data[[mi]]$switch, delta = delta[[mi]])
      } else {
        FB					<- FBalgCF(y2 = m.data[[mi]]$y2, m = m, pr = pr[[mi]], Mx = m.data[[mi]]$Mx, Mx2 = m.data[[mi]]$Mx2, gamma = gamma[[mi]], d = d, S2 = m.data[[mi]]$switch2, S = m.data[[mi]]$switch, delta = delta[[mi]], S3 = m.data[[mi]]$switch.sl)
      }
      B_star					<- FB[[2]]
      Occupancy				<- FB[[3]]
      N 						<- FB[[1]]
    ...
    for(i in 1:m){
      ...
      for (mi in 1:n.mice){
        ...
        RWout					<- mnl_RW_once(int1 = c_int[[i]][mi,], Obs = Trans2, n_cat = (m-1), mu_int_bar1 = mu_int_bar[[i]], V_int1 = V_int[[i]], scalar = scalar, candcov1 = candcov.comb)
        gamma[[mi]][i, -i]  	<- PD.mice[[mi]][iter, c((m*q_pr + 1+(i-1)*m):(m*q_pr + (i-1)*m + m))[-i]] <- RWout$prob
        naccept[mi, i]			<- naccept[mi, i] + RWout$accept
        c_int[[i]][mi,]		 	<- RWout$draw.int

        pr.candcov.comb			<- chol2inv(chol(m.data[[mi]]$pr.mhess[(1+(i-1)*(q_pr-1)):((q_pr-1)+(i-1)*(q_pr-1)),] + chol2inv(chol(V_pr.int[[i]]))))
        pr.RWout				<- mnl_RW_once(int1 = pr.c_int[[i]][mi,], Obs = cond.y[[mi]][[i]], n_cat = q_pr, mu_int_bar1 = mu_pr.int_bar[[i]], V_int1 = V_pr.int[[i]], scalar = pr.scalar, candcov1 = pr.candcov.comb)
        pr[[mi]][i,]			<- PD.mice[[mi]][iter, (1+(i-1)*q_pr):((i-1)*q_pr + q_pr)] <- pr.RWout$prob
        pr.naccept[mi, i]		<- pr.naccept[mi, i] + pr.RWout$accept
        pr.c_int[[i]][mi,]		<- pr.RWout$draw.int

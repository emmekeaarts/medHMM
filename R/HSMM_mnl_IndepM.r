...
#################################################################
# 				Hybrid Metropolis GIBBS SAMPLER					#
#################################################################


# Model parameters are sampled from distributions based on sampled state sequence. 
# The state sequence is sampled using the outcomes of the forward backward alghorithm: first run forward sequence, then backward sequence, 
...
# pi_ (rows i of gamma) 	~(idd) 	Multinomial logit(int) for every row of gamma
									# i.e., categorical distrition modeled by an multinomal intercept only logit model.  
...
# Yt1s:t2s(theta_i) (obs) 	~(idd) 	H(theta_Zs)
									# Conditional distribuiton for observations given the (super)state
...
# (theta_i, w_i) 			~ (idd)	H*G = the distribuiton of the duration of the states and the conditional distribuiton of the the observations given the states are independent of eachother. 


# For model, see p. 8 of Johnson and Willsky 2012, Bayesian nonparameteric hidden semi markov models
...
#	 so B_star[[1]] corresponds to t1s = 0. B_star denotes the most likley duration of the state that started at t1s.
# - Occupancy: The sum over each list of B_star, denotes the overall most likley state starting at t1s.
...
# the parameter switch (dummy indicatior indicating the start of a new act) and switch2 (dummy indicatior indicating the end of an act)

# int and pr_int are sampled using a independence metropolis sampler. 
# This requires a proposal distribuiton, which is set to be a multivariate t distribution with the covariance and delta obtained 
# from the maximum likelihood estimates (mle) of the multinomial logit model using the sampled states, and degrees of freedom nu 
...
# Gamma_Sampler: list containing the start values for the mle algorithm, int.mle0 and degrees of freedom for t distribuiton, nu. 
# Pr_Sampler: list containing the start values for the mle algorithm, pr.int.mle0 and degrees of freedom for t distribuiton, pr.nu.
...
HSMM_mnl_IndepM <- function(Data, Start_Val, Gamma_Sampler = NULL, Pr_Sampler = NULL, Gamma_Prior = NULL, Pr_Prior = NULL, D_Prior, Mcmc, Fast_vers = FALSE, Fast_vers_par = NULL){
...
# Initialize pr sampler
	if(is.null(Pr_Sampler)){
		pr.int.mle0	<- rep(0, q_pr-1)
		pr.nu 		<- 3 + (q_pr-1)
			} else {
		pr.int.mle0	<- Pr_Sampler$pr.int.mle
		pr.nu 		<- Pr_Sampler$pr.nu
	}
...
# Define arguments used in Mcmc algorithm		
	int <- int.star <- int.new <- mle <- int.mle <- mhess <- candcov <- vector("list", m)
	pr.int <- pr.int.star <- pr.int.new <- pr.mle <- pr.int.mle <- pr.mhess <- pr.candcov <- vector("list", m)
	naccept <- rep(0,m)
	pr.naccept <- rep(0,m)
...
# Run the MCMC algorithm
	...
	# with the sample path, updata gamma, pr and duration distribution
		...
		# Update gamma and pr using Independence metropolis step 
			...
			IndepMout			<- mnl_IndepM_once(int1 = int[[i]], Obs = Trans2, n_cat = (m-1), mu_int_bar1 = mu_int0, V_int1 = sigma_int0, nu.M1 = nu, candcov1 = candcov[[i]], int.star1 = int.star[[i]])
			gamma[i, -i] 		<- IndepMout$prob	
			naccept[i]			<- naccept[i] + IndepMout$accept
			int[[i]] 			<- IndepMout$draw.int
			
			pr.IndepMout		<- mnl_IndepM_once(int1 = pr.int[[i]], Obs = cond.y, n_cat = q_pr, mu_int_bar1 = mu_pr.int0, V_int1 = sigma_pr.int0, nu.M1 = pr.nu, candcov1 = pr.candcov[[i]], int.star1 = pr.int.star[[i]])
			pr[i, ]				<- pr.IndepMout$prob
			pr.naccept[i]		<- pr.naccept[i] + pr.IndepMout$accept
			pr.int[[i]]			<- pr.IndepMout$draw.int      			

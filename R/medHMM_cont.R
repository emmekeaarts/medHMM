...
medHMM_cont <- function(s_data, gen, xx = NULL, start_val, emiss_hyp_prior, mcmc, return_path = FALSE, print_iter, show_progress = TRUE,
    ...
    for(s in 1:n_subj){
        ...
        n_vary[s] <- n
        subj_data[[s]]	<- c(
            subj_data[[s]], n = n, list(gamma_converge = numeric(m), gamma_int_mle = matrix(NA_real_, m, (m - 1)),
                                        gamma_mhess = matrix(NA_real_, (m - 1) * m, (m - 1))),
            Mx = n,
            Mx2 = rep(n, m)
        )
    }
    ...
    d_mu0			<- dur_hyp_prior$d_mu0
    s2_0			<- dur_hyp_prior$s2_0
    alpha.sigma20	<- dur_hyp_prior$alpha.sigma20
    beta.sigma20	<- dur_hyp_prior$beta.sigma20
    alpha.tau20		<- dur_hyp_prior$alpha.tau20
    beta.tau20		<- dur_hyp_prior$beta.tau20
    ...
    logmu       <- mu_d_bar <- matrix(PD[1, ((n_dep * m * 2 + m * m + 1)) :((n_dep * m * 2 + m * m + m))], nrow = n_subj, ncol = m, byrow = TRUE)
    ...
    for (iter in 2 : J){
        ...
        for(s in 1:n_subj){
            ...
            delta <- get_delta(gamma[[s]], m)

            allprobs <- get_all1(x = subj_data[[s]]$y, emiss = emiss[[s]], gamma = gamma[[s]], n_dep = n_dep)
            ...
            Occupancy			<- FB[[3]]
            N 					<- FB[[1]]
            PD_subj[[s]][iter, m*n_dep*2 + m*m + m*2 + 1] <- llk <- sum(N)	# adjust index; we may need to do log-sum-ex

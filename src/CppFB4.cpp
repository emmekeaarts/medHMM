...
// still have to find out the difference with FB2.
//' @keywords internal
...
List FBalgCF(NumericVector y2, int m, NumericVector delta, NumericMatrix pr, int Mx, IntegerVector Mx2,NumericMatrix gamma, NumericMatrix d, IntegerVector S, IntegerVector S2, IntegerVector S3) {
  ...
  for (t = 0; t <= n-1; t++) {
    ...
    for (j = 0; j < m; j++) {
      if (t == 0) {
        Norm(j,0) = log(delta(j)) + log(pr(j, y2(0)));
      }
      else
      {
        Norm(j,t) = log(pr(j,y2(t))) + log(std::abs(exp(StateIn(j,t)) - exp(Forward(j, (t-1))) + exp(Norm(j, (t-1)))));
      }
      N(t) += exp(Norm(j,t));
    }
    N(t) = log(N(t));
    ...
    for (j = 0; j < m; j++) {
      ...
      if (t < n-1) {
        for (u = 1; u <= std::min(t+1, Mx2(j)); u++) {
          Observ += log(pr(j,y2(t-u+1))) - N(t-u+1);
          if (SShrev2(u-1) == 1) {
            if (u < t + 1) {
